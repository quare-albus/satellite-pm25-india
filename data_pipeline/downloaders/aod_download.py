...
#hardcoded path to 'config.json' file - Please Update this if your 'config.json' is in a different location or use the default path by keeping it in the same directory as the script.
config_file = "/home/victus7/Desktop/Personal-Project/AirPollution/config/mosdac_config.json"
...
records_per_day = search_params.get("records_per_day", 0)
boundingBox = search_params.get("boundingBox", "")
gId = search_params.get("gId", "")

# Configure pagination batch size for the search endpoint.
# The API supports a maximum of 100 records per call, so we batch requests accordingly.
page_size = 100
if count:
    try:
        page_size = int(count)
        if page_size <= 0:
            raise ValueError
        if page_size > 100:
            print("\n[WARNING] 'count' in config.json is greater than 100. The API returns a maximum of 100 records per call. Using 100 records per batch.")
            page_size = 100
    except ValueError:
        print("\n[WARNING] Invalid 'count' value in config.json. Using 100 records per batch.")
        page_size = 100

count = str(page_size)

# Calculate days per batch based on records per day
# If records_per_day is provided, calculate how many days fit in page_size records
days_per_batch = None
if records_per_day and records_per_day > 0:
    days_per_batch = max(1, page_size // records_per_day)
    print(f"\n[INFO] Records per day: {records_per_day} | Days per batch: {days_per_batch}")
...
def split_date_range(start_date_str, end_date_str, days_per_batch):
    """
    Splits a date range into chunks based on days per batch.
    Returns a list of tuples: [(start_chunk, end_chunk), ...]
    ...
    if start_date > end_date:
        print(f"\n[ERROR] startTime ({start_date_str}) is after endTime ({end_date_str})")
        return []
    ...
    while current_start <= end_date:
        current_end = current_start + timedelta(days=days_per_batch - 1)
...
def search_results(search_start_time=None, search_end_time=None):
    """Fetches all data from the search endpoint using pagination.""" 
    print()
    print("Searching Data for Provided Parameters...")
    
    # Use provided date range or fall back to global variables
    current_start_time = search_start_time if search_start_time else startTime
    current_end_time = search_end_time if search_end_time else endTime
    
    data = {"datasetId": datasetId, "startIndex": startIndex}
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            print(f"[INFO] Fetching {itemsPerPage} records per page.")
            return totalResults
...
def main():
    ...
    # If days_per_batch is configured, split date range into chunks
    date_chunks = []
    if days_per_batch:
        date_chunks = split_date_range(startTime, endTime, days_per_batch)
        if not date_chunks:
            print("Failed to split date range. Exiting.")
            return
        print(f"\n[INFO] Date range split into {len(date_chunks)} chunks")
    else:
        # Single chunk - use the provided date range
        date_chunks = [(startTime, endTime)]
    
    # # Get user input and authentication once
    # if len(date_chunks) == 1:
    #     total_files = search_results()  
    # else:
    #     # For multiple chunks, show summary
    #     print(f"\n[INFO] Processing {len(date_chunks)} date chunks...")
    #     total_files = 0
    #     for i, (chunk_start, chunk_end) in enumerate(date_chunks):
    #         chunk_results = search_results(chunk_start, chunk_end)
    #         print(f"[INFO] Chunk {i+1}/{len(date_chunks)} ({chunk_start} to {chunk_end}): {chunk_results} records")
    #         total_files += chunk_results
    ...
    for i, (chunk_start, chunk_end) in enumerate(date_chunks):
        if len(date_chunks) > 1:
            print(f"\n[INFO] Processing chunk {i+1}/{len(date_chunks)} ({chunk_start} to {chunk_end})...")
        
        # Update global startTime and endTime for this chunk
        startTime = chunk_start
        endTime = chunk_end
        
        # Get total files for this chunk
        chunk_total_files = search_results(chunk_start, chunk_end)
        
        download_complete, download_count, skip_count = fetch_and_download_data(chunk_total_files, access_token, refresh_token)
    ...
    if all_complete and len(date_chunks) > 0:
        print(f"\n{GREEN}All Chunks Downloaded Complete!{RESET}\n")
        total_time = end_time - start_time
        total_minutes = total_time / 60
        total_hours = total_time / 3600

        print(f"Total No. of Files Downloaded: {total_download_count}") 

        if (total_skip_count > 1):
            print(f"Files Skipped for Download: {total_skip_count}\n") 
